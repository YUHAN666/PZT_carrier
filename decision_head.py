...
def decision_head(x, y, class_num, scope, keep_dropout_head, training, data_format, momentum, mode, reuse=None, drop_rate=0.2, activation='relu'):
	...
	with tf.variable_scope(scope, reuse=reuse):
		...
		x = CBR(x, 32, 3, 2, training, momentum, mode, name='CBR1', padding='same', data_format=data_format, activation=activation, bn=True)
		x = CBR(x, 32, 3, 1, training, momentum, mode, name='CBR2', padding='same', data_format=data_format, activation=activation, bn=True)
		x = CBR(x, 64, 3, 2, training, momentum, mode, name='CBR3', padding='same', data_format=data_format, activation=activation, bn=True)
		x = CBR(x, 64, 3, 1, training, momentum, mode, name='CBR4', padding='same', data_format=data_format, activation=activation, bn=True)
		x = CBR(x, 64, 3, 2, training, momentum, mode, name='CBR5', padding='same', data_format=data_format, activation=None, bn=False)
